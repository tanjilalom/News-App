...
class Bdnews24Screen extends StatefulWidget {
  const Bdnews24Screen({super.key});

  @override
  State<Bdnews24Screen> createState() => _Bdnews24ScreenState();
}

class _Bdnews24ScreenState extends State<Bdnews24Screen> {
  static const _baseUrl = 'https://bdnews24.com';
  static const _accentColor = Color(0xFF1565C0);
  static const _pageUrl = 'https://bdnews24.com';
    ...
    try {
      ...
      for (final anchor
          in document.querySelectorAll(
              'a[href*="/bangladesh/"], a[href*="/world/"], a[href*="/politics/"]')) {
        final href = anchor.attributes['href'] ?? '';
        if (href.isEmpty) continue;
        final url = href.startsWith('http') ? href : '$_baseUrl$href';
        final title = TextUtils.cleanText(anchor.text);
        if (title.length < 10 || !seenLinks.add(url)) continue;
        final timeEl =
            anchor.parent?.querySelector('time, .time, .date');
        items.add(NewsItem(
            title: title,
            url: url,
